...
- [x] 后端 pytest：`.deps/backend-venv/bin/python -m pytest backend/tests -q`，12 passed。
- [x] 前端构建：`cd frontend && npm run build`，构建通过。
- [x] 前端 E2E：`cd frontend && npm run test:e2e`，4 passed。
...
- [x] 浏览器中可完成：填项目、开始面试、回答一轮、结束复盘；无 console error。
