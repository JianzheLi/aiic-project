...
1. 用户先选择训练入口：八股知识点、简历经历、手撕代码、完整模拟。
2. 八股模式选择知识分类；简历模式上传 PDF/DOCX/TXT 简历或粘贴文本；手撕代码模式选择 LeetCode 高频题或 AI 算子题；完整模拟要求简历输入。
3. AI 面试官按对应训练目标连续追问。
4. 用户用文字作答。
5. AI 给出结构化反馈、回答问题指出和下一轮改进建议。
6. 用户可以基于反馈继续追问或重练。

### 待定产品取舍

- [x] 输入升级为上传 PDF/DOCX/TXT 简历或粘贴简历文本，岗位 JD 可选；文字型 PDF 优先用 PyMuPDF + pypdf 提取文本层，扫描 PDF 才尝试本地 OCR。
- [x] 简历文件上限从固定 5MB 放宽为默认 25MB，并支持通过 `RESUME_MAX_BYTES` 配置。
...
- [x] 面试官追问强度调整为“真实但可回答”：不强要精确召回率/准确率等数字，优先问判断过程、日志线索和如果没记录会如何补指标。
- [x] 产品结构升级为三入口：首页先选择八股知识点、简历经历、手撕代码，三类训练使用独立界面和独立会话。
- [x] 八股训练新增分类：后端/数据库、搜广推、Agent/LLM、AI 算法；Agent 纯知识点归入八股，不再混在简历经历里。
- [x] 手撕代码新增内置题库：LeetCode 高频算法与 AI 算子/模型实现，首版采用“题目 + 思路/代码粘贴 + AI 静态评审”，不运行用户代码。
- [x] 新增 `/training/message` 三模式主接口，保留 `/interview/message` 作为简历追问兼容接口。
- [x] 新增完整模拟入口：前三个入口作为专项强化，完整模拟按简历深挖、八股基础、手撕代码和综合追问推进，并输出完整报告。

### MVP 方向草案

- 训练入口：首屏提供 4 个入口：八股知识点、简历经历、手撕代码、完整模拟。
- 八股流程：选择分类、开始知识点追问、回答后获得本轮反馈、结束并总结。
- 简历流程：上传/粘贴简历、选择追问场景、开始面试、连续追问、结束并总结。
- 手撕代码流程：选择题类、查看题目、粘贴思路和代码、获得静态评审、结束并总结。
- 完整模拟流程：上传/粘贴简历、按真实面试节奏回答项目/八股/手撕/综合追问、结束后生成完整面试报告。
- 反馈输出：总评、亮点、风险点、知识漏洞/项目可信度/代码问题、表达结构、可执行改进建议、下一轮练习题；完整模拟额外生成维度评分和训练计划。
...
- [x] 新增结构化训练题库：`backend/app/data/knowledge_topics.json` 与 `backend/app/data/coding_problems.json`。
- [x] 新增 `/training/message`，统一支持 `knowledge`、`resume`、`coding`、`full_mock` 四种训练模式。
- [x] 改造前端为四入口训练首页和独立训练界面。
- [x] 修复移动端/小屏训练后输入框不可达的问题：工作台在窄屏允许页面滚动，并在生成问题后自动滚动聚焦答题框。
...
- 2026-05-10：取消固定 5MB 简历上传限制，改为默认 25MB 的可配置上限，避免大 PDF/OCR 文件被过早拒绝。
- 2026-05-10：加强文字型 PDF 解析路径：先用 PyMuPDF 抽取文本层，再 fallback 到 pypdf，仍不足才进入 OCR。
...
- 2026-05-10：新增完整模拟入口，把三个专项强化串成一场完整技术面试，并在结束后生成结构化面试报告；同时修复小屏滚动导致无法触达输入框的问题。
