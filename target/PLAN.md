...
- [x] 新增完整模拟入口：前三个入口作为专项强化，完整模拟由 AI 根据简历和前序回答判断下一问方向；项目、简历相关八股、手撕代码和综合追问每个板块至少两轮，并输出完整报告。
...
- 完整模拟流程：上传/粘贴简历，由 AI 根据简历技术栈、目标岗位和回答状态决定下一问；覆盖项目/简历相关八股/手撕/综合追问，结束后生成完整面试报告。
