...
3. 每个板块至少连续或累计追问两轮；不要“问一个就跑”。如果某板块只问过一轮，除非回答完全无法继续，否则优先继续该板块。
4. 你需要根据简历、目标岗位和前序回答判断下一轮最该问什么；轮次编号只提供进度，不是固定脚本。
5. 八股基础必须和候选人的简历技术栈、项目风险或目标岗位相关，例如简历写 Redis 就追缓存一致性/击穿，写 RAG 就追 chunk/召回/rerank，写 MHA/模型就追 attention/shape/复杂度。
6. 手撕代码轮不运行用户代码，只要求候选人讲思路、复杂度、边界，并可粘贴核心代码；题目也要尽量贴近简历或目标岗位。
7. 如果候选人回答空泛，要追一个更可验证的切面：个人贡献、坏例、日志/指标、边界样例、复杂度或失败兜底。
8. 最终报告必须基于本轮对话证据，不要泛泛鼓励，不要编造用户没有说过的经历或数据。
""".strip()
...
维度评分必须覆盖：简历/项目可信度、简历相关八股基础、手撕代码、技术表达、承压追问、岗位匹配度。
过程回放里要说明四个板块是否都至少问了两轮；如果用户提前结束导致未覆盖满，要明确标注“覆盖不足”。
...
""".strip()
    ...
    coverage_policy = f"""
完整模拟最低覆盖要求：
- 简历经历深挖：至少 2 轮，追个人贡献、技术取舍、失败路径、可验证依据。
- 简历相关八股基础：至少 2 轮，必须从简历技术栈、项目风险或目标岗位中选题，不问无关八股。
- 手撕代码/算法思路：至少 2 轮，题目贴近目标岗位或简历技术方向，追思路、复杂度、边界样例。
- 综合追问/岗位匹配：至少 2 轮，基于前面暴露的薄弱点继续压测。

当前进度：第 {payload.round + 1} 轮，最多 {payload.max_rounds} 轮。
判断方式：
1. 先阅读全部对话，自己判断四个板块各问过几轮、哪个板块证据不足。
2. 如果某板块不足 2 轮，优先补足该板块；如果多个不足，选择最能影响面试结果的一个。
3. 如果四个板块都已至少 2 轮，再选择前序回答中最薄弱、最值得继续验证的方向。
4. 本轮只问一个主问题，输出中可以用“项目轮 / 八股轮 / 手撕轮 / 综合轮”标记当前板块，但不要解释内部计数。
""".strip()
...
首轮通常应从简历经历深挖开始，因为后续八股和手撕都要从简历技术栈和岗位方向延展。
...
请根据简历和前序回答，自主判断本轮最该问“项目轮 / 八股轮 / 手撕轮 / 综合轮”中的哪一个。
如果进入八股轮，必须和简历技术栈或目标岗位相关；如果进入手撕轮，题目也要贴近候选人的方向。
不要按固定脚本机械切换，不要一个板块只问一轮就跳走。
只输出面试官这一轮要问的话。
""".strip()
